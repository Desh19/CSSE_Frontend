...
// --- Constants (Expanded Waste Types) ---
// These are the types now conceptually supported by the backend model
...
// --- Utility Function to get Resident ID and Token ---
const getUserData = () => {
  try {
...
const SchedulePickup = () => {
  ...
  const [formData, setFormData] = useState({
    requestType: WasteTypeOptions[0].value, // Default to the first type
    description: '',
    scheduledDate: '',
  });
  const [location, setLocation] = useState(null); // Stores { latitude, longitude }
  const [status, setStatus] = useState('idle'); // idle | fetching_location | loading | success | error
  const [message, setMessage] = useState('');

  // --- Geolocation Effect ---
  useEffect(() => {
    if (!residentId) return; // Don't fetch location if user is not logged in
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          setStatus('idle'); // Location fetched, ready to submit
  ...
  const handleSubmit = async (e) => {
    ...
    // 1. Prepare the full description string including GPS data
    const locationString = `GPS: ${JSON.stringify(location)}`;
    const fullDescription = `${formData.description}`;

    // 2. Prepare the payload for the API
    const payload = {
      // NOTE: requestId is likely generated by the backend, so we don't send it.
      requestType: formData.requestType,
      description: fullDescription,
      scheduledDate: new Date(formData.scheduledDate).toISOString(),
      resident: residentId, // Sent as the user ID for the resident field
      location:locationString
    };
    
    try {
      // 3. Execute the API call using the provided service function
  ...
  return (
    <div className="max-w-4xl mx-auto my-12 p-6 md:p-12 bg-white shadow-2xl rounded-xl border border-gray-100">
      ...
      <form onSubmit={handleSubmit} className="space-y-8" disabled={!residentId}>
        ...
        {/* Waste Type Selection (Grid Layout) */}
        <div>
        ...
        {/* Location Status (Final Confirmation) */}
